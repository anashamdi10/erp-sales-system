            ...
            <div class="card-body">
                ...
                <form action="{{ route('admin.uoms.update',$data['id']) }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <select @if($total_counter_used> 0) disabled @endif class="form-control" name="is_master" id="is_master">
                            <option value="">اختر الوحدة  </option>
                            <option @if(old('is_master',$data['is_master'])==1) selected="selected" @endif value="1">وحده أب
                            </option>
                            <option @if(old('is_master',$data['is_master'])==0) selected="selected" @endif value="0">وحدة تجزئة
                            </option>
                        </select>
                        @error('is_master')
                        <span class="text-danger">{{$message}}</span>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="active" id="active">
                            ...
                            <option @if(old('active',$data['active'])==0) selected="selected" @endif value="0">لا
                            </option>
                        </select>
                        @error('active')
                        <span class="text-danger">{{$message}}</span>
