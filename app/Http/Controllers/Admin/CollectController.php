...
class CollectController extends Controller
{
    public function index()
    {
      ...
      // check_if_admim has shifts to collect 
        ...
        $accounts = get_cols_where(new AccountModel(), array("name", "account_number", "account_type"), array("com_code" => $com_code, "active"=>1, "is_parent" => 0), 'id', 'DESC');
        ...
        return view('admin.collect_tranaction.index', ['data' => $data, 'check_exsits_shifts' => $check_exsits_shifts ,'accounts' =>$accounts , 'mov_type'=>$mov_type , 'treasures'=> $treasures , 'users'=> $users]);
    }
    ...
    {   
        ...
        if ($request->ajax()) {
            ...
            if($accountData['account_type']== 2 ){
                $the_current_balance = refresh_account_blance_suppliers($account_number,new AccountModel(),new SuppliersModel(),
                                        new Treasuries_transactionModel(),new Suppliers_orderModel(),true);
                ...
                $the_current_balance = refresh_account_blance_customer($account_number, new AccountModel(),new Customer(),
                                        new Treasuries_transactionModel(),new Sales_invoices(),true);
                return view('admin.collect_tranaction.load_current_balance', ['the_current_balance' => $the_current_balance]);           
            }else{
                $the_current_balance = refresh_account_blance_general($account_number, new AccountModel(),
                                        new Treasuries_transactionModel() , true);
                return view('admin.collect_tranaction.load_current_balance', ['the_current_balance' => $the_current_balance]);                         
            }
    ...
    public function search(Request $request)
    {
        if ($request->ajax()) {
            ...
            return view('admin.collect_tranaction.ajax_search', ['data' => $data , 'total_collect_search'=>$total_collect_search]);
        }
    }
